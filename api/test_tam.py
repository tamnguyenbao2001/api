...
if str('../yolov8_tracking') not in sys.path:
    sys.path.append(str('../yolov8_tracking'))  # add yolov5 ROOT to PATH
if str('../yolov8_tracking/yolov8') not in sys.path:
    sys.path.append(str('../yolov8_tracking/yolov8')) 
if str('../yolov8_tracking/trackers/strongsort') not in sys.path:
    sys.path.append(str('../yolov8_tracking/trackers/strongsort'))
...
movenet_dir = "movenet_trt_saved_model"
movenet_trt = tf.saved_model.load(movenet_dir, tags = [tag_constants.SERVING])
movenet = movenet_trt.signatures["serving_default"]

recognizer_dir = "ActivityRecognition/xgb_activity_recognizer.onnx"
recognizer = onnxruntime.InferenceSession(recognizer_dir, providers = ['TensorrtExecutionProvider', 'CUDAExecutionProvider', "CPUExecutionProvider"])

tracking_method = 'strongsort'
tracking_config = '../yolov8_tracking/trackers/strongsort/configs/strongsort.yaml' 
...
def visualisation(frame, x1, y1, x2, y2):                              # Input: x1, x2, y1, y2
    ...
    outputs_t = movenet(cropped_img[tf.newaxis, ...])
            ...
            try:
                for i in range(len(outputs)):
                    with open('eee.txt', 'a') as f:
                        f.write(('%g ' * 8 + '\n') % (dataa[i,0],outputs[i,-3] ,outputs[i,-2],outputs[i,-1], outputs[i,0], outputs[i,1], outputs[i,2], outputs[i,3]))
            except: pass
            cnt +=1
            cv2.imshow("TAM",im0)
            cv2.waitKey(10)
