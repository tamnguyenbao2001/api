...
class StrongSORT(object):
    ...
    def update(self, dets,  ori_img):
        ...
        detections = [Detection(bbox_tlwh[i], confs[i], feature) for i, feature in enumerate(
            features)]
        ...
        for track in self.tracker.tracks:
            ...
            conf = track.conf
            outputs.append(np.array([x1, y1, x2, y2, track_id, class_id, conf], dtype=np.float32))
        if len(outputs) > 0:
